      ...
      .join(
        (enter) =>
          enter
            ...
            .style('padding', '10px 15px') // Padding inside the tooltip
            .style('background-color', 'rgba(0, 0, 0, 0.5)') // Semi-transparent white background
            .style('backdrop-filter', 'blur(10px)') // Blurry background
            .style('-webkit-backdrop-filter', 'blur(10px)') // For Safari
            .style('box-shadow', '0 4px 6px rgba(0, 0, 0, 0.2)') // Subtle shadow for depth
            .style('text-align', 'center')
            .style('color', 'white') // Light font color
            .style('font-size', '18px') // Adjust font size as needed
            .style('font-weight', 'lighter') // Lighter font weight
            .style('max-width', '250px') // Maximum width of the tooltip
  ...
  set_highlit_points(data: StructRowProxy[]) {
    ...
    sel
      ...
      .join(
        (enter) => 
          enter
            .append('circle') // Glowing background circle
